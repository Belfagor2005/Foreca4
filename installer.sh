...
version='1.3.6'
changelog='\nComplete Migrations from scraper to Api'

TMPPATH=/tmp/Foreca4-install
FILEPATH=/tmp/Foreca4-main.tar.gz

if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/Foreca4
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/Foreca4
fi

echo "Starting Foreca4 installation..."
...
echo "Downloading Foreca4..."
wget --no-check-certificate 'https://github.com/Belfagor2005/Foreca4/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "Failed to download Foreca4 package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "Failed to extract Foreca4 package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/Foreca4-main/usr/lib/enigma2/python/Plugins/Extensions/Foreca4" ]; then
    cp -r "$TMPPATH/Foreca4-main/usr/lib/enigma2/python/Plugins/Extensions/Foreca4"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from standard plugin directory"
elif [ -d "$TMPPATH/Foreca4-main/usr/lib64/enigma2/python/Plugins/Extensions/Foreca4" ]; then
    cp -r "$TMPPATH/Foreca4-main/usr/lib64/enigma2/python/Plugins/Extensions/Foreca4"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/Foreca4-main/usr" ]; then
    cp -r "$TMPPATH/Foreca4-main/usr"/* /usr/ 2>/dev/null
    echo "Copied entire usr structure"
else
    echo "Could not find plugin files in extracted archive"
    echo "Available directories:"
    find "$TMPPATH" -type d -name "*Foreca4*" | head -10
    cleanup
    exit 1
fi
